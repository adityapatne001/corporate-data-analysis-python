# 📊 Corporate Data Analysis & Reporting Tool – Python

**Tools Used:** Python, Pandas, Matplotlib, Tkinter  
**Dataset:** Sample Business Sales Data (Excel / CSV)  
**Files:** Python source code, raw dataset, output screenshots

---

## 🔍 Project Overview

This project is an end-to-end **Python-based data analysis and reporting tool** designed to simulate real-world business reporting workflows. It enables users to load raw CSV or Excel datasets, clean and aggregate data, generate summary reports and visualize insights through charts.

The primary focus of the project is on **data cleaning, aggregation logic, automated reporting and visualization**, demonstrating practical usage of Python for data analytics rather than GUI development.

---

## 🧠 Key Features

### 🔹 Data Ingestion & Cleaning
- Supports **CSV and Excel** input files
- Handles safe numeric conversion (e.g. `"12,000"` → `12000`)
- Cleans text columns and removes invalid records

### 🔹 Analytical Reporting
- Dynamic **GroupBy and aggregation** operations:
  - Sum
  - Mean
  - Count
  - Min
  - Max
  - Median
- Automated report generation using Pandas
- Exportable reports in **Excel (.xlsx)** and **CSV (.csv)** formats

### 🔹 Data Visualization
- Visual insights generated using Matplotlib:
  - Bar Chart
  ![bar_chart](./outputs/bar_chart.png)
  - Column Chart
  - Line Chart
  - Pie Chart
- Charts can be exported as **PNG images** for presentations and reporting

### 🔹 Business-Oriented Workflow
- Interactive selection of grouping, aggregation and value columns
- Preview reports and charts before exporting
- Designed to reflect real analyst-driven reporting tools used in organizations

This project was completed as part of a certified Power BI course by [Mr. Satish Dhawale – Founder and CEO @ Skill Course].



